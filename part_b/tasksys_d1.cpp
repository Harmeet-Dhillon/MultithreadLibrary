...
void TaskSystemParallelThreadPoolSleeping::waiting(){
    while(!done.load(std::memory_order_acquire)){
            ...
            lock_guard<mutex> store_lock(mtx_store);
            for(auto&[k,v]:store){
                ...
                if(v.empty()){
                    ...
                    vector<int> temp2;
                    lock_guard<mutex> ready_lock(mtx_ready);
                    for(auto e:v){
...
void TaskSystemParallelThreadPoolSleeping::atomic_flow(state& s){
        while(true){
            ...
            if(currscore==s.total_tasks-1){
                ...
                if(rec+1==dem)done.store(true,memory_order_release);
...
void TaskSystemParallelThreadPoolSleeping::execute(int threadID) {
    while (!done.load(std::memory_order_acquire)) {
...

        {
            ...
            int gidx = global_counter.load(std::memory_order_acquire);

            if (gidx < static_cast<int>(toprocess.size())) {
                task = &toprocess[gidx];
            }
        }
